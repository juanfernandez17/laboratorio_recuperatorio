...
static Node* getNode(LinkedList* this, int nodeIndex)
{
	...
	if(this != NULL && nodeIndex >= 0 && nodeIndex < sizeList)
	{
...
int ll_add(LinkedList* this, void* pElement)
{
    ...
    {
    	sizeList = ll_len(this);
    	returnAux = addNode(this, sizeList, pElement);
    }
    return returnAux;
}
...
void* ll_get(LinkedList* this, int index)
{
    ...
    if(this != NULL && index >= 0 && index < sizeList)
    {
    	indexNode = getNode(this, index);

    	if(indexNode != NULL)
    	{
    		returnAux = indexNode->pElement;
    	}
    }
    return returnAux;
}
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
	...
	if(this != NULL && from >= 0 && from < sizeList && to >= 1 && to <= sizeList && from < to)
	{
...
int ll_sort(LinkedList* this, int (*pFunc)(void* ,void*), int order)
{
    ...
    if(this != NULL && pFunc != NULL && (order == 1 || order == 0))
    {
		...
		{
			elementUno = ll_get(this, i);

			for(int j = i + 1; j < sizeList; j++)
			{
				elementDos = ll_get(this, j);
				returnFunc = pFunc(elementUno, elementDos);

				if((order == 1 && returnFunc > 0) || (order == 0 && returnFunc < 0))
				{
					ll_set(this, i, elementDos);
					ll_set(this, j, elementUno);
					elementUno = ll_get(this, i);
				}
			}
		}
...
LinkedList* ll_filter(LinkedList* this, int(*fn)(void* el))
{
...
int ll_map(LinkedList* this, int(*fn)(void* el))
{
	int returnAux;
	int sizeList;
	void* element;
